...
namespace renderer {
    ...
    //% blockId=place_3d_line
    //% block="place line color %color from x1 %x1 y1 %y1 height1 %h1 to x2 %x2 y2 %y2 height2 %h2"
    //% baseSize.defl=1
    //% anchor.defl=ScaleAnchor.Bottom
    //% inlineInputMode=inline
    export function place3dLine(color: number, x1: number, y1: number, h1: number, x2: number, y2: number, h2: number) {
        let a = worldToScreen(x1, y1, h1)
        let b = worldToScreen(x2, y2, h2)
        if (Math.abs(x1 - px) < zFar && Math.abs(y1 - py) < zFar && Math.abs(x2 - px) < zFar && Math.abs(y2 - py) < zFar) {
            if (a[2] < 0 && b[2] < 0) {
                // line is in front of camera
                mySprite.image.drawLine(a[0], a[1], b[0], b[1], color)
            } else if (a[2] >= 0 && b[2] >= 0) {
                // do nothing, line is fully behind camera
            } else {
                // One endpoint is in front of camera, one is behind camera. Subdivide it.
                //console.log("x1=" + x1 + " y1=" + y1 + " x2=" + x2 + " y2=" + y2)
                if (Math.abs(x2 - x1) + Math.abs(y2 - y1) < 10) {
                    // Line segment is too short, skip it
                    return
                }
                let midX = (x1 + x2) / 2
                let midY = (y1 + y2) / 2
                let midH = (h1 + h2) / 2
                place3dLine(color, x1, y1, h1, midX, midY, midH)
                place3dLine(color, midX, midY, midH, x2, y2, h2)
            }
        }
    }
