...
function endRace () {
    game.splash("Congrats!", "You Got " + placement + " Place!")
    game.reset()
}
function place () {
...
trackImage = img`
    ...
    `
raceStarted = false
maps = [tilemap`level1`, tilemap`level57`]
arrayx2 = []
arrayy2 = []
arrayh2 = []
carAngularVelocity = 0
...
coinXMap = []
coinYMap = []
collisionMaps = [tilemap`level5`, tilemap`level30`]
CPUPX = []
CPUPY = []
CPUAngle = [
0,
            ...
            if (tiles.tileAtLocationEquals(tiles.getTileLocation(CPUPX[index] / 16, CPUPY[index] / 16), assets.tile`myTile0`)) {
                if (CPUpos[index] == 0) {
                    ...
                    for (let value of tiles.getTilesByType(assets.tile`myTile1`)) {
                        if (dis > Math.sqrt((CPUPX[index] - value.x) * (CPUPX[index] - value.x) + (CPUPY[index] - value.y) * (CPUPY[index] - value.y))) {
                            dis = Math.sqrt((CPUPX[index] - value.x) * (CPUPX[index] - value.x) + (CPUPY[index] - value.y) * (CPUPY[index] - value.y))
                            CPUTempX[index] = value.x + randint(-4, 4)
                            CPUCheckPoints[index] = value.y + randint(-4, 4)
                        }
                    }
                }
                CPUpos[index] = 1
...
game.onUpdate(function () {
    ...
    if (tiles.tileAtLocationEquals(tiles.getTileLocation(px / 16, py / 16), assets.tile`myTile9`) || tiles.tileAtLocationEquals(tiles.getTileLocation(px / 16, py / 16), assets.tile`myTile25`)) {
        if (PlayerCP == 7) {
            PlayerLapCount += 1
            if (PlayerLapCount > 2) {
                endRace()
